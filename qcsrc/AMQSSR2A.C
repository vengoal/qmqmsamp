...
/******************************************************************************/
/*                                                                            */
/* Module name: AMQSSR2A.C                                                    */
/*                                                                            */
/* Description: Sample C program that subscribes using RFH2 format messages   */
/*              to the supplied topic using the default subscription point    */
/*              and displays any arriving publications. The subscriber is     */
...
/* Function:                                                                  */
/*                                                                            */
/*   AMQSSR2A is a sample C program for MQ Publish/Subscribe, it              */
/*   demonstrates subscribing using RFH2 format messages.                     */
/*                                                                            */
/*   A subscription message for the supplied topic, is put to MQ's            */
/*   Publish/Subscribe control queue (SYSTEM.BROKER.CONTROL.QUEUE) using the  */
/*   default subscription point (SYSTEM.BROKER.DEFAULT.SUBPOINT) and the      */
/*   supplied subscriber queue, which is then checked for the reply messages  */
/*   returned.                                                                */
/*                                                                            */
...
/*   publication contains text in the user data area following the RFH2       */
/*   header.                                                                  */
/*                                                                            */
...
/*   MQOPEN subscriber queue for publications or replies                      */
/*   Define the MQRFH2 for the subscription                                   */
...
/*   While publications are being received:                                   */
...
/******************************************************************************/
/*                                                                            */
/* AMQSSR2A takes 3 input parameters:                                         */
/*                                                                            */
...
int main(int argc, char *argv[])
{
  ...
  /* MQI structures                                                          */
  /***************************************************************************/
  MQOD ctlQOD={MQOD_DEFAULT};                   /* control queue object desc */
  MQOD subQOD={MQOD_DEFAULT};                   /* subsc.. queue object desc */
  MQMD getMD={MQMD_DEFAULT};                    /* get message descriptor    */
  MQGMO getMO={MQGMO_DEFAULT};                  /* get message options       */
  /***************************************************************************/
  /* MQI variables                                                           */
  /***************************************************************************/
  MQHOBJ hCtlQObj;                              /* hdl to control Q object   */
  MQHOBJ hSubQObj;                              /* hdl to subsc.. Q object   */
  MQCHAR topic[MAX_TOPIC_LENGTH];               /* topic                     */
  MQCHAR subscriberQName[MQ_Q_NAME_LENGTH+1];   /* subscriber queue name     */
  MQCHAR QMgrName[MQ_Q_MGR_NAME_LENGTH+1]="";   /* default Q manager name    */
  MQINT32 options;                              /* options                   */
  MQINT32 compCode;                             /* completion code           */
  MQINT32 openCQCompCode=MQCC_FAILED;           /* MQOPEN control queue CC   */
  MQINT32 openSQCompCode=MQCC_FAILED;           /* MQOPEN subscri.. queue CC */
  ...
  PMQRFH2 pRFH2;                                /* ptr to MQRFH2             */
  /***************************************************************************/
  /* Other variables                                                         */
  /***************************************************************************/
  BOOL registered=FALSE;                        /* registered as subscriber  */
  ...
  strncpy(subscriberQName, argv[2], MQ_Q_NAME_LENGTH); /* get the reply queue name */
  ...
  /***************************************************************************/
  /* Open the control queue for output - commands put to this queue          */
  /***************************************************************************/
  if (rc == OK)
  {
    strcpy(ctlQOD.ObjectName, "SYSTEM.BROKER.CONTROL.QUEUE");
    options = MQOO_OUTPUT + MQOO_FAIL_IF_QUIESCING;
    MQOPEN(hCon, &ctlQOD, options, &hCtlQObj, &openCQCompCode, &reason);
    mqCheck("MQOPEN", openCQCompCode, reason);
  } /* endif */

  /***************************************************************************/
  /* Open the subscriber queue for input - pubs and replies got from this Q  */
  /***************************************************************************/
  if (rc == OK)
  {
    strcpy(subQOD.ObjectName, subscriberQName);
    options = MQOO_INPUT_SHARED + MQOO_FAIL_IF_QUIESCING;
    MQOPEN(hCon, &subQOD, options, &hSubQObj, &openSQCompCode, &reason);
    mqCheck("MQOPEN", openSQCompCode, reason);
  } /* endif */

  /***************************************************************************/
  /* Register as a subscriber to the supplied topic                          */
  /***************************************************************************/
  if (rc == OK)
  {
    /************************************************************************/
    /* Set any registration options required for registering here           */
    /************************************************************************/
    /* options = MQREGO_CORREL_ID_AS_IDENTITY +                             */
    /*           MQREGO_FULL_RESPONSE +                                     */
    /*           MQREGO_LOCAL +                                             */
    /*           MQREGO_PUBLISH_ON_REQUEST_ONLY;                            */
    options = 0;

    rc = SendPubSubCmd(MQPSC_REGISTER_SUBSCRIBER,
                       hCtlQObj,               /* command object handle     */
                       hSubQObj,               /* reply object handle       */
                       subscriberQName,        /* reply queue name          */
                       topic,                  /* topic                     */
                       options,                /* options                   */
                       NULL,                   /* ptr to user data          */
                       0);                     /* user data length          */
    if (rc == OK)
    {
      registered = TRUE;
      printf("Registered as a subscriber for topic '%s'\n", topic);
    } /* endif */
  } /* endif */

  /***************************************************************************/
  /* Set up some things for the MQGET loop                                   */
  /***************************************************************************/
  if (rc == OK)
  {
    msgBufLen = sizeof(msgBuf) - 1;            /* leave room for terminator */

    getMO.Options = MQGMO_WAIT + MQGMO_CONVERT + MQGMO_NO_SYNCPOINT;
    getMO.WaitInterval = PUBLISH_WAIT_TIME;
    /************************************************************************/
    ...
    getMO.MatchOptions = MQGMO_NONE;
  } /* endif */

  /***************************************************************************/
  /* Get and display publications from our subscriber queue                  */
  /***************************************************************************/
  while (rc == OK)
  {
    /************************************************************************/
    /* MQGET sets Encoding and CodedCharSetId to the values in the message  */
    /* returned, so these fields should be reset to the default values      */
    /* before every call, as MQGMO_CONVERT is specified .                   */
    /************************************************************************/
    getMD.Encoding = MQENC_NATIVE;
    getMD.CodedCharSetId = MQCCSI_Q_MGR;

    MQGET(hCon, hSubQObj, &getMD, &getMO, msgBufLen, msgBuf, &msgLen, &compCode, &reason);
    ...
    if (reason == MQRC_NONE)
    {
      ...
      /*********************************************************************/
      pRFH2 = (PMQRFH2) msgBuf;               /* set the RFH2 pointer      */

      if (memcmp(getMD.Format, MQFMT_RF_HEADER_2, MQ_FORMAT_LENGTH) != 0)
      {
        rc = NOT_OK;
        printf("MD.Format not RFH2 for message received on the subscriber queue, "
               "MD.Format: %.8s\n", getMD.Format);
      } /* endif */
      ...
      {
        rc = NOT_OK;
        printf("StrucId not RFH for message received on the subscriber queue, "
               "StrucId: %.4s\n", pRFH2->StrucId);
      } /* endif */
      ...
      {
        rc = NOT_OK;
        printf("Version not MQRFH_VERSION_2 for message received on the subscriber queue, "
               "Version: %d\n", pRFH2->Version);
      } /* endif */

      /*********************************************************************/
      /* The only part of the publication that we are interested in, in    */
      /* this sample is the string data that has been added to the message */
      /* after the fixed length part of the MQRFH2 and NameValueLength and */
      /* NameValueData pairs. For example:                                 */
      /*                                                                   */
      /* ----------------------                                            */
      ...
      if (rc == OK)
      {
        ...
        /******************************************************************/
        if (msgLen > pRFH2->StrucLength)
        {
          msgBuf[msgLen] = '\0';            /* add terminator            */
          pUserData = (PMQCHAR) pRFH2 + pRFH2->StrucLength;
          printf("message <%s>\n", pUserData);
        }
        else
        {
  ...
  /***************************************************************************/
  /* Deregister as a subscriber to the supplied topic                        */
  /***************************************************************************/
  if (registered)
  {
    /************************************************************************/
    /* Set any registration options required for deregistering here         */
    /************************************************************************/
    /* options = MQREGO_CORREL_ID_AS_IDENTITY +                             */
    /*           MQREGO_FULL_RESPONSE;                                      */
    options = 0;

    rc = SendPubSubCmd(MQPSC_DEREGISTER_SUBSCRIBER,
                       hCtlQObj,               /* command object handle     */
                       hSubQObj,               /* reply object handle       */
                       subscriberQName,        /* reply queue name          */
                       topic,                  /* topic                     */
    ...
    if (rc == OK)
      printf("Deregistered as a subscriber for topic '%s'\n", topic);
  } /* endif */

  /***************************************************************************/
  /* Close the subscriber queue if opened                                    */
  /***************************************************************************/
  if (openSQCompCode != MQCC_FAILED)
  {
    MQCLOSE(hCon, &hSubQObj, MQCO_NONE, &compCode, &reason);
    mqCheck("MQCLOSE", compCode, reason);
  } /* endif */

  /***************************************************************************/
  /* Close the control queue if opened                                       */
  /***************************************************************************/
  if (openCQCompCode != MQCC_FAILED)
  {
    MQCLOSE(hCon, &hCtlQObj, MQCO_NONE, &compCode, &reason);
    mqCheck("MQCLOSE", compCode, reason);
  } /* endif */
